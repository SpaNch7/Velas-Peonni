    ...
    public Cliente salvar(Cliente cliente) {
        try {
            ...
            if (cliente.getEmail() == null || cliente.getEmail().isBlank()) {
                throw new RuntimeException("Email do cliente não pode ser vazio");
            }
            // Regra 1: email duplicado - seguro para insert (id null) e update
            boolean emailDuplicado = repository.findAll().stream()
                    .anyMatch(c -> c.getEmail().equalsIgnoreCase(cliente.getEmail())
                            && (cliente.getId() == null || !c.getId().equals(cliente.getId())));
            if (emailDuplicado) {
